...
const { validateCreateUser, getAllUsers, getUSerById, deleteUser } = require('../service/userService');
const { invalidPassword } = require('../utils/validations');

const { CONFLICT, CREATED, BAD_REQUEST, OK, NOT_FOUND, NO_CONTENT } = require('../utils/statusCodeHandler');
...
userController.delete('/me', verifyToken, rescue(async (request, response) => {
  const { id } = request.user.dataValues;

  response.status(NO_CONTENT.code).json({});
